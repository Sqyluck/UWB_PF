...
#endif
        sprintf(debug, "DIST: %2.3fm", distance);
        println(debug);
#if CAL_ANT_DLY
        sprintf(debug, "ant : %d", inst->txAntennaDelay);
        println(debug);
        if (distance > 5.12) {
        	inst->txAntennaDelay++;
        	inst->rxAntennaDelay++;
        	inst->antennaDelayChanged = 1;
        } else if (distance < 5.06) {
        	inst->txAntennaDelay--;
        	inst->rxAntennaDelay--;
        	inst->antennaDelayChanged = 1;
        } else {
        	println("ANT DELAY OK");
        }
...
void instance_config(instanceConfig_t *config, sfConfig_t *sfConfig)
{
    ...
    char debug[50];
    println("--- CONFIG ---");
    sprintf(debug, " - chan: %d", inst->configData.chan);
    println(debug);
    sprintf(debug, " - preCode: %d", inst->configData.rxCode);
	println(debug);
	sprintf(debug, " - prf: %d", inst->configData.prf);
	println(debug);
	sprintf(debug, " - dataRate: %d", inst->configData.dataRate);
	println(debug);
	sprintf(debug, " - txPreambLength: %d", inst->configData.txPreambLength);
	println(debug);
	sprintf(debug, " - rxPAC: %d", inst->configData.rxPAC);
	println(debug);
	sprintf(debug, " - nsSFD: %d", inst->configData.nsSFD);
	println(debug);
	sprintf(debug, " - phrMode: %d", inst->configData.phrMode);
	println(debug);
	sprintf(debug, " - sfdTO: %d", inst->configData.sfdTO);
	println(debug);
	...
	if ((2 == otprev) || (3 == otprev))  // board is calibrated with TREK1000 with antenna delays set for each use case)
	{
				...
				|| (dly == 0xffffffff))
		{
			...
			println("nothing programed trek");
		}
		else
		{
			inst->txAntennaDelay = (dly >> (16*(mode & 0x1))) & 0xFFFF;
			println("something1 programed");
		}
	...
	else // assume it is older EVK1000 programming.
	{
		...
		{
			println("nothing programed not trek");
	...
	sprintf(debug, "%d, %d", inst->txAntennaDelay, inst->txAntennaDelay);
	println(debug);
