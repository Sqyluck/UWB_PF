...
int dwt_spicswakeup(uint8 *buff, uint16 length)
{
    if(dwt_readdevid() != DWT_DEVICE_ID) // Device was in deep sleep (the first read fails)
    {
    	...
    	while (length > 0) {
    		if (length > 100) {
                dwt_readfromdevice(0x0, 0x0, 100, buff); // Do a long read to wake up the chip (hold the chip select low)
                length -= 100;
    		} else {
                dwt_readfromdevice(0x0, 0x0, length, buff); // Do a long read to wake up the chip (hold the chip select low)
                length = 0;
...
uint8 dwt_getxtaltrim(uint8 value)
{
    // The 3 MSb in this 8-bit register must be kept to 0b011 to avoid any malfunction.

    uint8 reg_val = dwt_read8bitoffsetreg(FS_CTRL_ID, FS_XTALT_OFFSET);
    return (reg_val & 0x1F);
}
